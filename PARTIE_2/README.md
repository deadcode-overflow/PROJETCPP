 ...
 *Projet C++ : Cryptage et décryptage*
 ...
 Le programme est répartie en 9 fichiers :
 ...
 ## main.cpp :
 ---
 Le fichier `main.cpp` appelle la fonction `huffman` qui lance le programme principal et le codage de huffman.

  ## huffman.h et huffman.cpp :
 ---
 Le fichier `huffman.cpp` definie la fonction `huffman` qui contient permet de coder un texte donné par l'utilisateur et de créer l'arbre d'Huffman.
  ...
  * Elle possède une méthode `décomposer`, qui décomposse l'arbre en deux arbres distincts.
    ...
    - Dans le cas contraire, on va utiliser deux autres fonctions qui sont `recupererGauche` et `recupererDroite` qui sont des fonctions récursives et qui vont ajouter dans tout le sous-arbre `gauche` ou `droite` respectivement de l'arbre qu'on veut décomposer dans les arbres passées en parametres. Ces fonctions font appel à la méthode `ajouter`.
 ...
 * Elle possède une méthode `fusionner_huffman`, qui fusionne deux arbres binaire donnés en paramètre de fonction à l'aide de constructeur par copie et qui additionne la frequence des deux arbres dans la nouvelle racine. Le résultat de la fusion est ensuite retourné.