 ...
 *Projet C++ : Cryptage et décryptage*
  ...
  * `bin/` : qui une fois compiler, contient le fichier exécutable `main.exe`.

 Le programme est répartie en 8 fichiers :
 ...
 * Elle possède une méthode `nombre_element`, qui calcule le nombre de sommet d'un arbre binaire en vérifiant d'abord si l'arbre est vide, dans ce cas, elle renvoie 0, sinon, elle ajoute un au résultat finale et rapelle la fonction sur le sous-arbre gauche et le sous-arbre droit.
 ...
 ---
 Pour exécuter pregramme du projet, il vous suffit de taper `./bin/main.exe` dans le terminal.
...
Le build est répartie en 3 dossiers :
 ...
 ---
 Pour exécuter le programme Qt du projet, il vous suffit de taper `./build/Main` dans le terminal.
