 ...
 * Elle possède une méthode `supprimer`, qui supprime un sommet dans l'arbre binaire.
   Cette méthode fonctionne comme ceci : 
    ...
    - A ce stade, on a notre sommet à supprimer qui se trouve soit à gauche, soit à droite. Dans les deux cas, 3 possibilités s'offrent à nous.
        ...
        On s'est servi de ces deux fonctions de la façon suivante : On détermine la valeur max du sous arbre gauche,on regarde si le sommet max a luimêmen fils ou 0 : 
 ...
 * Elle possède une méthode `fusion`, qui fusionnne deux arbres en un unique arbre binaire :
   Cette fonction prend en parametre l'arbre à ajouter dans l'arbre qui apellent la méthode.
...
* Elle possède une méthode `hauteur`, qui calcule la hauteur d'un arbre.
* Elle possède une méthode 'nombre_element', qui calcule le nombre de sommet d'un arbre binaire.
